...
?>
<div class="shop-element bx-<?=$arParams['TEMPLATE_THEME']?>" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product">
					...
					<?=($arParams['IMAGE_RESOLUTION'] === '1by1' ? 'product-item-detail-slider-block-square' : '')?>"
                            ...
                            <div class="product-item-detail-slider-images-container" data-entity="images-container">
                                ...
                                foreach($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $fileID){
                                    $arResult['MORE_PHOTO'][] = CFile::GetPath($fileID);
                                }
                                foreach ($arResult['MORE_PHOTO'] as $key => $asPicture)
                                {
                                    ?>
                                    <div class="product-item-detail-slider-image<?=($key == 0 ? ' active' : '')?>"
                                         data-entity="image" data-id="image_<?=$key?>">
                                        <img src="<?=$asPicture?>" <?=($key == 0 ? ' itemprop="image"' : '')?>>
                                    </div>
                                    <?
                                }
                                ?>
                        ...
                        <div class="product-item-detail-slider-controls-block" id="<?=$itemIds['SLIDER_CONT_ID']?>">
                            ...
                            foreach ($arResult['MORE_PHOTO'] as $key => $photo)
                            {
                                ...
                                <div class="product-item-detail-slider-controls-image<?=($key == 0 ? ' active' : '')?>"
                                     data-entity="slider-control" data-value="image_<?=$key?>">
                                    <img src="<?=$photo?>">
                                </div>
                                <?
                            }
    ...
    <div class="q-container container-inner">
        <div class="left_block">
            <img src="<?=$arResult['DETAIL_PICTURE']['SRC'];?>" alt="<?= $arResult['DETAIL_PICTIRE']['ALT'];?>">
        </div>
        <div class="right_block">
            <div class="title_item"><h2 class="title"><?=$name?></h2></div>
            ...
            <div class="options_item">
                <?php
                if($arResult['CATALOG_WIDTH'] || $arResult['CATALOG_LENGTH'] || $arResult['CATALOG_HEIGHT']):?>
                    <div class="option_item params_size">
                        <span class="icons"></span>
                        <span class="value">
                                <?php
                                $size = $arResult['CATALOG_WIDTH'];
                                if($arResult['CATALOG_LENGTH']){
                                    $size .= ' X '.$arResult['CATALOG_LENGTH'];
                                }
                                if($arResult['CATALOG_HEIGHT']){
                                    $size .= ' X '.$arResult['CATALOG_HEIGHT'];
                                }
                                $size .= ' cm';
                                echo $size;
                                ?>
                ...
                <? foreach ($arResult['DISPLAY_PROPERTIES'] as $arOneProp)
                {
                    ...
                    if('gem' == $arOneProp['CODE']) continue;
                    ?>
                    <div class="option_item">
                        <span class="title"><? echo $arOneProp['NAME']; ?></span>
                        <span class="value">
                                <?
                                echo (
