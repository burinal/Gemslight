;(function(jqLoc) {
    ...
    $(document).ready(function() {
        ...
        DOMHighestZIndex = findHighestZIndex();
    ...
    function getMicrosoftTranslatorSupportedLangs() {
        if ( arLocaleParams.microsofttranslate_sup_langs ) return false;
    ...
    $.fn.addLocaleFormHandlers = function() {
        ...
        this.find('.g_translate, .y_translate, .microsoft_translate').click(function(e){
            ...
            $(this)
                .closest('.locale_popup').hide().prev().removeClass('active');
    ...
    $(document).on('click', '.locale_popup .deactivate', function(event) {
        ...
        $this
            .closest('.locale_popup')
            .hide()
            .siblings('.locale-click-arrow.active')
            .removeClass('active');
            ...
            for ( var bx = 0; bx < $bxAreaBlocks.length; bx++ ) {
                ...
                if ( thisId ) {
                    var $scThis = $('body').find('script:contains("\'parent\':\''+ thisId +'\'")'),
                        ...
                        dataText = '', componentText = '', componentTemp = '';

                    if ( ~scTextOrg.indexOf("'"+thisId+"'") ) {
                        dataText = getPathTextWithScriptText(scTextOrg);
                        componentText = getCompWithScriptText(scTextOrg);
                        componentTemp = getTempWithScriptText(scTextOrg);

                        if ( dataText ) {
                            $this.data('local_pathsave', dataText);

                            if ( componentText == 'bitrix:menu' ) {
    ...
    function getTempWithScriptText(text) {
        if ( !text ) return false;

        var regTemp = /(template_id|templateId)\=(.*?)(\&|\;|\\)/i,
            regRez = regTemp.exec(text),
            temp = regRez?regRez[2]:false;

        return temp?decodeURIComponent(temp):false;
    }

    function getCompWithScriptText(text) {
        if ( !text ) return false;

        var regComp = /(component_name)\=(.*?)(\&|\;)/i,
            regRez = regComp.exec(text),
            comp = regRez?regRez[2]:false;

        return comp?decodeURIComponent(comp):false;
    }
     ...
     * @param {string} [text] - �������� ������
     * */
    function getPathTextWithScriptText(text) {
        if ( !text ) return false;

        var regPath = /(\;path|\?path|\&path)\=(.*?)(\&|\;)/ig,
            paths = [],
            result;

        while (result = regPath.exec(text)) {
            if (result[2] && result[2].search(/\.php$/i) != '-1' ) paths.push(decodeURIComponent(result[2]));
        }

        return paths.length?JSON.stringify(uniqueArr(paths)):false;
    }
     ...
     * @param {string} [rEnd] - ������ �����
     * */
    function getAllPathWithScriptText(text, rStart, rEnd) {
        if ( !text || !rStart || !rEnd ) return false;

        var paths = [],
            i = 0;

        while ( ~text.indexOf(rStart) ) {
            text = text.substring(text.indexOf('BX.CEditorDialog'));
            text = text.substring(text.indexOf(rStart));
            paths.push(decodeURIComponent(text.substring(rStart.length, text.indexOf(rEnd))));
            text = text.substring(text.indexOf(rEnd));
            ++i;
        }

        return JSON.stringify(onlyPHPLinks(uniqueArr(paths)));
    }
     ...
     * @return {array}
     * */
    function onlyPHPLinks(arr) {
        var result = [];

        for (var i = 0; i < arr.length; i++) {
            if (~arr[i].indexOf('.php')) result.push(arr[i]);
        }
    ...
    $(function() {
        ...
        function parseTemplateScript( text ) {
            ...
            if ( result[1] ) {
                component_name = unescape(result[1]);
            }
         ...
         * �������� ���� � ��������� bx_incl_area, ���� �� ����� �� ������, ����� ����� ����� �� ��������
         * */
        function getBxInclAreaDataInf(object, text) {
            var $object = $(object),
                objAttr = $object.attr('id'),
                $bxInclArea = text&&objAttr&&~objAttr.search(/^bx_incl_area/)?$object.add($object.parents('[id ^= "bx_incl_area"]')):$object.parents('[id ^= "bx_incl_area"]'),
                dataPath = $bxInclArea.data('local_pathsave'),
                localBxName = $bxInclArea.data('local_bx_name'),
                localBxTemp = $bxInclArea.data('local_bx_temp'),
                $obj;

            for ( var i = 0; i < $bxInclArea.length; i++ ) {
                var $eq = $bxInclArea.eq(i);

                dataPath = $eq.data('local_pathsave');

                if ( dataPath ) {
                    localBxName = $eq.data('local_bx_name');
                    localBxTemp = $eq.data('local_bx_temp');
                    $obj = $eq;
            ...
            return {
                'local_pathsave': dataPath,
                'local_bx_name': localBxName,
                'local_bx_temp': localBxTemp,
                'obj': $obj
            };
        }
        ...
        $(document).on('mouseup.locale', 'p, div, span, td, input, img, a, select, select > option', function(e) {
            ...
            if ($objclone.is('input')) {
                ...
                $('body').append('<input type="text" value="" name="focus_loc_inp" id="focus_loc_inp">');
            ...
            var getDataBxArea = objTextType=='text'?getBxInclAreaDataInf($this, 1):getBxInclAreaDataInf($this);

            //console.log(getDataBxArea);

            pathToParse = getDataBxArea['local_pathsave'];
            parse['component'] = getDataBxArea['local_bx_name'];
            parse['template'] = getDataBxArea['local_bx_temp'];
            ...
            showPopupTimeSet = objTextType=='text'?0:500;
                ...
                function(data){
                    ...
                    if (data.FORM){
                        ...
                        $innerBlock.on('click','.lang-trigger',function() {
                            var $this = $(this),
                                rel = $this.attr('rel'),
                                $addLangWrapp = $($('.js-block-lang').get(0)),
                                $thisLangBlock = $('.locale-block.block-'+rel);

                                $addLangWrapp.append($thisLangBlock.show());
                        ...
                        if ( data.error && (data.errorType == 'inside') ) {
                            ...
                            if ( textParseHTMLArea && (data.FOUND != 'NOT_FOUND') ) {
                                for ( var ar = 0; ar<textParseHTMLArea.length; ar++ ) {
                                    ...
                                    if ( $(textParseHTMLArea[ar]).children().length ) {
                                        tagAmount += $(textParseHTMLArea[ar]).children().length;
                                    }
                                }
        ...
        function openTranslateWindow(obj, text) {
            ...
            toMoreTimeSending = sendLocaleData({'text':text,'translator_name':translator_name.name},function(data){
                ...
                $popupInner.find('select.change-langs').change(function() {
                    ...
                    $(this)
                        .parents('.select-wrap:first')
                        .find('.flag-container-default-lang > span:first')
                        .attr("class",'ico-flag-' + $(this).val().toUpperCase());
         ...
         */
        function openLocaleWin(obj, loader, pushClass, text, addClassPopup) {
            text = text || '';
            loader = loader || false;
            pushClass = pushClass || '';
                ...
                toMoreTimeSending = sendLocaleData(
                    ...
                    function(data) {
                        ...
                        if ( data.FORM ) {
                            ...
                            $PCPopupInnerBlock
                                .addKeyupLocaleHandler()
                                .addLocaleFormHandlers();

                            if ( $(obj).hasClass('system') ) {
                                $PCPopupInnerBlock
                                    .find('form:first')
                                    .prepend("<input type='hidden' name='is_system' value='Y'>");
                            }
                            ...
                            $PCPopupInnerBlock.on('click','.lang-trigger',function() {
                                ...
                                if ( $PCPopupInnerBlock.hasClass('big') ) {
                                    $PCPopupInnerBlock.find('.locale-block:visible').removeClass('last');
                                }
        ...
        $('a[title]').each(function() {
            if ( $(this).attr('title').indexOf('<i class=locale_mes') > -1 ){
                ...
                obj
                    .html('<loc title="'+obj.text()+'"></loc>')
                    .addClass('locale_title_translated');
                    ...
                    var menuItems = [
                        {
                            ...
                            ONCLICK: function() {
                                ...
                                openLocaleWin(t, killLoader, 'text_change_popup', text, 'notClosePopup');
                            }
                        }
                    ];
        ...
        var shiftKeyClicking = false;

        $(document).on('click', 'a', function(event) {
            if ( shiftKeyClicking ) return false;

            var aHref = $(this).attr('href').removeSpacesLocal();

            if ( aHref[0] == '#' || ~aHref.indexOf('javascript:void(0)') ) return true;

            if ( event.shiftKey ) {
                document.location.href = aHref;

                shiftKeyClicking = true;
    ...
    $(document).on('mousedown', '.PCPopup.popupLNMenu .resize.top-right', function(c) {
        ...
        if ( !$popupTextAreas.data('height') ) {
            $popupTextAreas.data('height', popupTextAreasHeight);
        }

        if ( !$searchPopup.data('height') ) {
            $searchPopup.data('height', searchPopupHeight);
        }
    ...
    function backToWindowVisiblePopup($popup) {
        ...
        if ( moveArrowLeft ) {
            $popupArrow.css('left', moveArrowLeft);
        }
    }
    ...
    function findHighestZIndex() {
        var maxIndex = 1000,
            $indexHeigEls = jQuery('*')
                ...
                .filter(function(){ return +($(this).css('z-index')) > maxIndex; });

        $indexHeigEls.each(function() {
    ...
    $.fn.PCPopup = function($formObj, options) {
        ...
        DOMHighestZIndex = findHighestZIndex() + 100;

        // ����� ����� �������? ������ ��� ��-�� firebug'�, ���� � �������� ������� ������� ������� firebug ��� z-index
        if ( DOMHighestZIndex > 1000000000 ) DOMHighestZIndex = 1000000000;

        if ( DOMHighestZIndex > settings.ZPosition ) {
            settings.ZPosition = DOMHighestZIndex;
            settings.bgZPosition = DOMHighestZIndex - 10;
        }
        ...
        if ( settings.popupWidth && parseInt(settings.popupWidth) > 0 ) {
            $htmlPopup.css('width',settings.popupWidth);
        }

        if ( settings.classes ) {
            $htmlPopup.addClass(settings.classes);
        }

        if ( settings.ides ) {
            $htmlPopup.attr('id', settings.ides);
        }
        ...
        if ( $formObj.length ) {
            ...
            // ���� Popup �������� �� ��������� ��������� ����
            if ( (elPos.doc.top+elInfo.height+$htmlPopup.outerHeight(true)) > xEls.height ) {
                settings.position='bottom';
            }
            ...
            if ( (elParams['place'] == 'bottom' && !settings.position) || settings.position=='bottom' ) {
                ...
                $popupArrow
                    .addClass('bx-core-popup-menu-angle-bottom')
                    .css({'left':bpopupArrowLeft});
            ...
            if ( (elParams['place'] == 'top' && !settings.position) || settings.position=='top' ) {
                ...
                $popupArrow
                    .addClass('bx-core-popup-menu-angle')
                    .css({'left':tpopupArrowLeft});
            ...
            if ( settings.close ) {
                ...
                $closeObj.on('click', function(event) {
                    if ( $htmlPopup.hasClass('nonClose') || $htmlPopup.hasClass('notClosePopup') ) return false;
            ...
            // ���������� ����� ��� �������
            var beckToPopup = function($popup, $button) {
                if ( !$popup.length || !$button.length ) return false;

                var wh = $(document).height(),
                    arrowLeft = parseInt($popup.find('.arrow').css('left')),
                    popupPosition = elementInfo.position($popup),
                    popupPosWidthPol = popupPosition.param.width / 2,
                    popupButton = elementInfo.position($button),
                    popupBtnCenterLeft = popupButton.doc.left + popupButton.param.width / 2,
                    popupPushLeft = popupBtnCenterLeft - popupPosWidthPol + (popupPosWidthPol - arrowLeft);
                ...
                var xEls = {
                    ...
                    xX = popupButton.doc.left + (popupButton.param.width / 2),
                    xY = popupButton.doc.top + (popupButton.param.height / 2);

                $CPPopupOverlay.css({
                    'height': wh
                });

                cpCoolSpot(xX, xY, xEls); // ��� �������� ������ ������ ������ ������
            };

            $(window).bind('resize', function() {
                beckToPopup($htmlPopup, $this);
            });
