...
elseif(!($USER->CanDoOperation('edit_php') || $USER->CanDoFileOperation('fm_lpa', $arPath)) && (HasScriptExtension($path) || substr(CFileman::GetFileName($path), 0, 1)=="."))
	$strWarning = GetMessage("LC_FILEMAN_FILEVIEW_PHPERROR");
...
<?if(strlen($strWarning) <= 0):?>
	...
	{
		if(false && $USER->CanDoOperation('fileman_edit_existent_files') && $USER->CanDoFileOperation('fm_edit_existent_file',$arPath))
		{
			$aDDMenuEdit = array();
			$aDDMenuEdit[] = array(
				"TEXT" => GetMessage("LC_FILEMAN_FILEVIEW_EDIT_AS_TEXT"),
				"ACTION" => "window.location='fileman_file_edit.php?".$addUrl."&site=".urlencode($site)."&path=".urlencode($path)."';",
			);
			if($USER->CanDoOperation('edit_php'))
			{
				$aDDMenuEdit[] = array(
					"TEXT" => GetMessage("LC_FILEMAN_FILEVIEW_EDIT_AS_PHP"),
					"ACTION" => "window.location='fileman_file_edit.php?".$addUrl."&site=".urlencode($site)."&path=".urlencode($path)."&full_src=Y';"
				);
			}
			$aDDMenuEdit[] = array(
				"TEXT" => GetMessage("LC_FILEMAN_FILEVIEW_EDIT_AS_HTML"),
				"ACTION" => "window.location='fileman_html_edit.php?".$addUrl."&site=".urlencode($site)."&path=".urlencode($path)."';"
			);

			$aMenu[] = array(
				"TEXT" => GetMessage("LC_EDIT"),
				"TITLE" => GetMessage("LC_EDIT"),
				"MENU" => $aDDMenuEdit,
				//"ICON" => "btn_edit"
			);
		}
	...
	if(false && ($USER->CanDoFileOperation('fm_download_file', $arPath) && !(HasScriptExtension($path) || substr(CFileman::GetFileName($path), 0, 1)==".")) ||false &&  $USER->CanDoOperation('edit_php'))
	{
		$aMenu[] = array(
			"TEXT" => GetMessage("LC_FILEMAN_FILEVIEW_DOWNLOAD"),
	...
	$id = GetFileName($path);
	if(false && $USER->CanDoFileOperation('fm_rename_file', $arPath))
	{
		$aMenu[] = array(
	...
	if(false && $USER->CanDoFileOperation('fm_delete_file', $arPath))
	{
		$aMenu[] = array(
			"TEXT" => GetMessage("LC_FILEMAN_FILE_DELETE"),
			"LINK" => "javascript:if(confirm('".GetMessage("LC_FILEMAN_FILE_DELETE_CONFIRM")."')) window.location='/bitrix/admin/fileman_admin.php?ID=".urlencode(urlencode(GetFileName($path)))."&action=delete&".$addUrl."&site=".urlencode($site)."&path=".urlencode(urlencode($folder_path))."&".bitrix_sessid_get()."';",
			"TITLE"	=> GetMessage("LC_FILEMAN_FILE_DELETE"),
			//"ICON" => "btn_delete"
		);
	}
	...
	$aTabs = array(
		array("DIV" => "edit1", "TAB" => GetMessage('LC_FILEMAN_VIEW_TAB'), "ICON" => "fileman", "TITLE" => GetMessage('LC_FILEMAN_VIEW_CONT')),
	);
	...
	<?if ($fileTypeParent=="text"):?>
		<tr class="heading">
			<td colspan="2"><?= GetMessage('LC_FILEMAN_VIEW_CONT') ?></td>
		</tr>
	...
	<?elseif($fileTypeParent=="image"):?>
		...
		<tr><td colspan="2"><?echo CFile::ShowImage($path, 600, 600, "border=0", "", true, GetMessage("LC_FILEMAN_FILEVIEW_ENLARGE"))?></td></tr>
	<?endif?>
	</tr>
<?
$tabControl->EndTab();
	...
	if(typeof(jQuery) == "undefined"){
		...
		script.addEventListener('load', function(){
			$(document).ready(function() {
				...
				var $spantext = $('span:contains('+HASH+')').get(1),
					$prevAllEl = $($spantext).prevAll(),
					$nextAllEl = $($spantext).nextAll();

				$prevAllEl.eq(0).addClass('hash-text-bg');
				$prevAllEl.eq(1).addClass('hash-text-bg');
				$($spantext).addClass('hash-text-bg');
				$nextAllEl.eq(0).addClass('hash-text-bg');
				$nextAllEl.eq(1).addClass('hash-text-bg');
				$nextAllEl.eq(2).addClass('hash-text-bg');
			});
		}, false);
	} else {
		$(document).ready(function() {
